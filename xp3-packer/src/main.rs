...
fn main() {
    ...
    let out = File::create(&args[2]).unwrap_or_else(|_| panic!("Cannot open {} for read", &args[1]));
    ...
    add_all_file(
        &mut writer,
        &PathBuf::new().join(&args[1]),
        &PathBuf::new().join(&args[1]),
    )
    .expect("Cannot add files");
